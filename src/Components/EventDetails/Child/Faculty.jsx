...
function Faculty({ event }) {
  ...
  useEffect(() => {
    readDocuments("Faculty")
      .then((data) => setfaculty(data.map(({ email }) => email)))
      .catch((err) => toast.error("Error while fetching faculty"))
  }, [])
  ...
  const availableFacultyMap = faculty
    .filter((user) => !cordinators.includes(user))
    .map((user, index) => {
      return (
        <div
          className="check_list_item"
  ...
  return (
    <div className="faculty_parent">
      ...
      <div className="input_box">
        ...
        <Button
          action={() => setaddToggled((e) => !e)}
          text={addToggled ? "Disable" : "Add"}
        />
        <SearchBar
          style={{ width: "35vw" }}
